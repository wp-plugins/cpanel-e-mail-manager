...
define('EMAILMGR_CURRENT_VERSION', "1.2.3");
...
function emailmanager_version_check() {
	if (get_site_option('emailmanager_version') != EMAILMGR_CURRENT_VERSION) {
...
function emailmanager_upgrade() {
	global $wpdb;
	$currentversion = get_site_option('emailmanager_version');
...
function emailmanager_output_list() {
	...
	if ( isset( $_GET['action'] ) && !empty( $_GET['action'] ) && isset( $_POST['uservalue'] ) && !empty( $_POST['uservalue'] ) && isset( $_POST['user'] ) && !empty( $_POST['user'] ) ) {
		...
		if ($_GET["action"] == "add") {
			...
			emailmanager_create_email($userid, $firstname, $lastname, $username, $email, $customformat, $customalias, $customquota, $custompass);
			$output .= '<div class="updated">
